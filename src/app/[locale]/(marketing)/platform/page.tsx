          ...
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 max-w-5xl">
            {capabilities.map((cap, i) => (
              <div key={i} className="border-t border-[var(--color-ink-subtle)] pt-4">
                <dt className="font-display font-semibold text-[16px] tracking-[-0.01em]">{cap.name}</dt>
        ...
        <Container width="wide">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 max-w-5xl">
              ...
              <ul className="flex flex-col gap-4">
                {platformChannels.live.map((channel) => (
                  <li key={channel.name} className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-display font-semibold text-[15px]">{channel.name}</p>
                      <p className="text-[14px] text-[var(--color-ink-muted)]">{pick(channel.note)}</p>
                    </div>
                  </li>
                ))}
                {platformChannels.dated.map((channel, i) => (
                  <li key={i} className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-display font-semibold text-[15px] text-[var(--color-ink-muted)]">{pick(channel.name)}</p>
                      <p className="text-[14px] text-[var(--color-ink-muted)]">{pick(channel.note)}</p>
                    </div>
                  </li>
                ))}
            ...
            <div>
              ...
              <p className="mt-8">
                <Link href="/trust" className="font-medium text-[14px] text-[var(--color-bangladesh-green)] hover:underline underline-offset-4">
