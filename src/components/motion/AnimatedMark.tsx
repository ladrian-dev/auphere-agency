...
/**
 * Premium hero mark animation — three phases, designed to make a visitor stop
 * scrolling without hijacking attention from the headline copy:
 *
 *  1. CONSTELLATION TRACE (~2.6s)
 *     A bright comet enters at the start of the path and traces the outer
 *     silhouette. Behind it, the entire mark's contour (all subpaths in
 *     parallel) draws in via stroke-dashoffset, glowing in mountain-meadow.
 *     The mark exists as a dim "ghost" stroke at low opacity throughout, so
 *     the eye knows where the form is going.
 *
 *  2. RADIAL FILL REVEAL (~0.9s)
 *     A circular mask grows from the dead center outward, flooding the mark
 *     with caribbean-green fill — feels like the silhouette catches fire from
 *     the inside.
 *
 *  3. AMBIENT SHIMMER LOOP (every ~11s)
 *     A soft diagonal band of light crosses the filled mark in ~1.6s, then
 *     fades. Repeats. Subtle enough not to distract from headline copy, alive
 *     enough to keep the hero from feeling static.
 *
 * Reduced motion: renders a static filled mark, no animation.
 */
export function AnimatedMark({
  ...
  const shouldAnimate = !reducedMotion && (immediate || inView);
  ...
  // Outer silhouette path — held by ref so we can sample points for the comet.
  const outlinePathRef = useRef<SVGPathElement>(null);
  const [outlineLength, setOutlineLength] = useState(0);

  useEffect(() => {
    if (outlinePathRef.current) {
      setOutlineLength(outlinePathRef.current.getTotalLength());
    }
  }, []);

  // Single source of truth for the trace progress — drives both the trail's
  // pathLength and the comet's position so they stay in sync.
  const traceProgress = useMotionValue(0);

  const cometCx = useTransform(traceProgress, (v) => {
    if (!outlinePathRef.current || !outlineLength) return -9999;
    return outlinePathRef.current.getPointAtLength(v * outlineLength).x;
  });
  const cometCy = useTransform(traceProgress, (v) => {
    if (!outlinePathRef.current || !outlineLength) return -9999;
    return outlinePathRef.current.getPointAtLength(v * outlineLength).y;
  });
  const cometOpacity = useTransform(traceProgress, [0, 0.04, 0.94, 1], [0, 1, 1, 0]);

  const trailControls = useAnimationControls();
  const fillMaskControls = useAnimationControls();
  const shimmerControls = useAnimationControls();

  // ── Phase orchestration ─────────────────────────────────────────────────
  useEffect(() => {
    if (!shouldAnimate || !outlineLength) {
      // Reduced-motion / SSR fallback: show the filled mark, hide the ghost.
      trailControls.set({ pathLength: 1, opacity: 0 });
      fillMaskControls.set({ r: REVEAL_MAX_RADIUS });
      traceProgress.set(1);
      return;
    }

    let cancelled = false;
    trailControls.set({ pathLength: 0, opacity: 0 });
    fillMaskControls.set({ r: 0 });
    traceProgress.set(0);

    (async () => {
      // PHASE 1 — comet trace + contour glow.
      // The trail path snaps to opacity quickly so the line has time to draw,
      // then pathLength rides traceProgress so the trail builds *behind* the
      // comet, not faster.
      trailControls.start({
        opacity: 1,
      ...
      await Promise.all([
        animate(traceProgress, 1, {
          duration: traceDurationSeconds,
          ease: [0.42, 0, 0.58, 1], // mild ease-in-out so the comet doesn't snap
        }),
        trailControls.start({
          pathLength: 1,
          transition: {
      ...
      // PHASE 2 — radial fill reveal flooding from center outward.
      await fillMaskControls.start({
        r: REVEAL_MAX_RADIUS,
        transition: { duration: fillRevealSeconds, ease: [0.32, 0.72, 0, 1] },
      });
      // Once filled, the trail glow is redundant — fade it under the fill.
      trailControls.start({
        opacity: 0,
        transition: { duration: 0.6, ease: 'easeOut' },
      });
      if (cancelled || !loopShimmer) return;

      // PHASE 3 — ambient shimmer loop.
      // Each pass: gradient slides from -180% to 180% across the mark, then
      // we wait the remainder of the interval before the next pass.
      const passDuration = 1.6;
      const restMs = Math.max(0, (shimmerIntervalSeconds - passDuration) * 1000);
      while (!cancelled) {
        shimmerControls.set({ x: -1.8 * VIEWBOX });
        await shimmerControls.start({
          x: 1.8 * VIEWBOX,
          transition: { duration: passDuration, ease: 'easeInOut' },
        });
        if (cancelled) return;
    ...
    >
      <defs>
        ...
        {/* Diagonal shimmer gradient — transparent → bright → transparent.
            Tilted via gradientTransform; the band's translation is animated
            separately on the wrapping <motion.g>. */}
        <linearGradient
          id={shimmerGradId}
          x1="0"
          y1="0"
      ...
      {/* Layer 1 — DIM GHOST: very faint stroke of the full mark so the eye
          knows where the form is going while the comet traces it. */}
      <path
        d={MARK_PATH_FULL}
        fill="none"
        stroke={COLORS.ghost}
      ...
      {/* Layer 2 — TRAIL: contour drawing behind the comet. */}
      <motion.path
        d={MARK_PATH_FULL}
        fill="none"
        stroke={COLORS.trail}
      ...
      {/* Outer silhouette as an invisible ref — used to sample comet points. */}
      <path
        ref={outlinePathRef}
        d={MARK_PATH_OUTLINE}
        fill="none"
      ...
      {/* Layer 3 — FILL: the final filled mark, revealed by a growing radial
          mask once the trace completes. */}
      <path
        d={MARK_PATH_FULL}
        fill={COLORS.fill}
        mask={`url(#${fillMaskId})`}
      ...
      {/* Layer 4 — SHIMMER: diagonal band swept across the filled mark on a
          loop. Clipped to the silhouette so it never leaks outside. */}
      {loopShimmer && (
        <g clipPath={`url(#${shimmerClipId})`}>
          <motion.g animate={shimmerControls} initial={{ x: -1.8 * VIEWBOX }}>
            <rect
              x={-VIEWBOX}
              y={-VIEWBOX}
              width={3 * VIEWBOX}
      ...
      {/* Layer 5 — COMET: the leading particle. Two circles, the back one
          blurred to give the glow halo, the front one sharp. Position bound
          to traceProgress via getPointAtLength on the outer silhouette. */}
      <motion.circle
        cx={cometCx}
        cy={cometCy}
        r={28}
